$(function () {
    ...
    $('table').find(':checkbox').on('change', function () {
        ...
        if ($(this).closest('form').find(':checkbox:checked').length) {
            $(this).closest('form').find('.actions').find('button:not([data-ignore])').removeAttr('disabled');
        } else {
            $(this).closest('form').find('.actions').find('button:not([data-ignore])').attr('disabled', 'disabled');
        }
    });
});
;