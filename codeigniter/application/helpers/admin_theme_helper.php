 ...
 */
function filePartial($file = '', $ext = 'php')
{
	$CI = & get_instance();
 ...
 * @return 	void
 */
function addAdminMenuPlace($key, $place)
{
	...
	$CI = get_instance();
	$CI->template->menuOrder = array_merge(array_slice($CI->template->menuOrder, 0, $place, true), array($key), array_slice($CI->template->menuOrder, $place, count($CI->template->menuOrder)-1, true));

}

function changefileType($string = 'image/png')
{
	list($type, $ext) = explode('/', $string);
