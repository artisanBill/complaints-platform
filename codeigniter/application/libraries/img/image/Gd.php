...
class Gd
{
    ...
    /**
     * @var $git app\libraries\image\driver\Gif
     */
    private $gif;
    ...
    public function open($imgname)
    {
        ...
        if ('gif' == $this->info['type']) {
            include (APPPATH . 'libraries/img/image/Gif.php');
     ...
     * @throws Exception
     */
    public function thumb($width, $height, $type = LE_THUMB_SCALING)
    {
        if (empty($this->im)) {
     ...
     */
    public function water($source, $locate = LE_WATER_SOUTHEAST)
    {
        //资源检测
        if (empty($this->im)) {
        ...
        switch ($locate) {
            /* 右下角水印 */
            case LE_WATER_SOUTHEAST:
                $x = $this->info['width'] - $info[0];
                $y = $this->info['height'] - $info[1];
                break;

            /* 左下角水印 */
            case LE_WATER_SOUTHWEST:
                $x = 0;
                $y = $this->info['height'] - $info[1];
                break;

            /* 左上角水印 */
            case LE_WATER_NORTHWEST:
                $x = $y = 0;
                break;

            /* 右上角水印 */
            case LE_WATER_NORTHEAST:
                $x = $this->info['width'] - $info[0];
                $y = 0;
                break;

            /* 居中水印 */
            case LE_WATER_CENTER:
        ...
        do {
            ...
            //平铺
            if (LE_WATER_COVERED === $locate) {
                $w_nember = ceil($this->info['width'] / $info[0]);
                $w_nember = $w_nember + 1;
                $h_nember = ceil($this->info['height'] / $info[1]);
                $h_nember = $h_nember + 1;
                for ($k = 0; $k < $w_nember; $k++) {
                    $x = ($this->info['width'] / 2 - $info[0] / 2) / $info[0];
                    $x = abs(floor($x) - $x) - 1;
                    $x = ($k == 0 ? $x * $info[0] : $x * $info[0] + $k * $info[0]);

                    for ($i = 0; $i < $h_nember; $i++) {
                        $y = ($this->info['height'] / 2 - $info[1] / 2) / $info[1];
                        $y = abs(floor($y) - $y) - 1;
                        $y = ($i == 0 ? $y * $info[1] : $y * $info[1] + $i * $info[1]);
                ...
                // for ($k = 0; $k < $w_nember; $k++) {
                //     for ($i = 0; $i < $h_nember; $i++) {
                //         imagecopy($src, $this->im, 0, 0, $k * $info[0], $i * $info[1], $info[0], $info[1]);
                //         imagecopy($src, $water, 0, 0, 0, 0, $info[0], $info[1]);
                //         imagecopymerge($this->im, $src, $k * $info[0], $i * $info[1], 0, 0, $info[0], $info[1], 100);
                //     }
                // }
            } else {
                imagecopy($src, $this->im, 0, 0, $x, $y, $info[0], $info[1]);
    ...
    public function text($text, $font, $size, $color = '#00000000',
        $locate = LE_WATER_SOUTHEAST, $offset = 0, $angle = 0) {
        //资源检测
        if (empty($this->im)) {
